...
def set_channel(iface, channel):
	if isinstance(channel, int):
		...
		subprocess.check_output(["sudo", "iw", iface, "set", "channel", str(channel)])
	else:
		# Channels represented as strings with extra info (e.g "11 HT40-")
		subprocess.check_output(["sudo","iw", iface, "set", "channel"] + channel.split())
...
def start_ap(iface, channel, beacon=None, ssid=None, interval=1000, dtim_period=1):
	"""
	...
	cmd = ["sudo","iw", "dev", iface, "ap", "start", ssid, str(chan2freq(channel)), "20", str(interval), str(dtim_period), "head", raw(beacon).hex()]
	if after_tim != None:
	...
	subprocess.check_output(["sudo","ifconfig", iface, "up"])
...
def start_virtual_interface(iface, viface):
	...
	# subprocess.check_output(["sudo", "iwconfig", "wlan4", "channel", "6"])
	# subprocess.check_output(["sudo", "iw", "wlan0", "set","channel", "6"])
